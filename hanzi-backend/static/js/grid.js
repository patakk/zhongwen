...
function updateCounterTitle(){
    ...
    // document.getElementById('grid-title').textContent = `${namesmap[currentDeck]} (${deckLength} words)`;  
    // if(useAllDecks){
    //     document.getElementById('grid-title').textContent = `${deckLength} words`;  
    // }
    // else{
    //     document.getElementById('grid-title').textContent = `${namesmap[currentDeck]} (${deckLength} words)`;
    // }
    // document.getElementById('grid-title').textContent = `${namesmap[currentDeck]} (${charCounter})`;
    if(inputdeck){
        currentDeck = inputdeck;
        inputdeck = null;
    }

    document.getElementById('grid-title').textContent = `${inputdecks[currentDeck].name}`;  
...
async function getInputDecks(callback = null) {
    if (decksFetched){
        initGridPage();
    }
    const [customPromise, staticPromise] = [
        fetch('./get_custom_cc').then(res => res.ok ? res.json() : Promise.reject('custom fetch failed')),
        fetch('./get_static_cc').then(res => res.ok ? res.json() : Promise.reject('static fetch failed')),
    ];

    try {
        const [customDecks, staticDecks] = await Promise.all([customPromise, staticPromise]);
        inputdecks = { ...customDecks, ...staticDecks };
        decksFetched = true;
        if (callback) callback();
    } catch (err) {
        console.error('Failed to fetch input decks:', err);
    }
}
...
function initGridPage() {
    ...
    let dm = document.getElementById('deckMenu');
    let fm = document.getElementById('fontMenu');

    if(ev1 !== null){
        dm.removeEventListener('click', ev1);
    }
    if(ev2 !== null){
        fm.removeEventListener('click', ev2);
    }
    if(ev3 !== null){
        document.removeEventListener('click', ev3);
    }
    if(dm){
        dm.removeEventListener('click', ev1);
        ev1 = dm.addEventListener('click', function(event) {
    ...
    if(fm){
        ev2 = fm.addEventListener('click', function(event) {
    ...
    if(true){
        ev3 = document.addEventListener('click', function(event) {
            let ds = document.getElementById('deckSubmenu');
            let fs = document.getElementById('fontSubmenu');
            if (!event.target.closest('#deckMenu') && ds) {
                ds.classList.remove('active');
            }
            if (!event.target.closest('#fontMenu') && fs) {
    ...
    ev4.forEach(ev => {
        ev.element.removeEventListener('click', ev.event);
    });
    document.querySelectorAll('.deck-change').forEach(function(deckOption) {
        let ev = deckOption.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        ev4.push({'event': ev, 'element': deckOption});
    });

    document.querySelectorAll('.font-change').forEach(function(fontOption) {
        let ev =  fontOption.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        ev4.push({'event': ev, 'element': fontOption});
    });
